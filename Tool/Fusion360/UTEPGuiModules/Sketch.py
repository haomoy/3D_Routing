...
def run(context, Names, Chs, Heights, Ps, Xs, Ys, CP, filename, extension, Ns):
    ui = None
    try:
        global app
...
def sketches():
    ...
    try:
        ...
        z = (plane.y)
        #y = -(plane.z) #This one was already commented
...
#                    smallestY = float(Cross[h][1])/10.0
        ...
        for i in range(0, len(CompNames)):#For each component in the list of component names...
            ...
            for p in range(0, len(Pins[i])):
                ...
                #Adding pins to components
                startPoint = adsk.core.Point3D.create(startX+X1, startY+Y1, z)
                endPoint = adsk.core.Point3D.create(startX+X2, startY+Y2, z)
                Pinlines.addByTwoPoints(startPoint, endPoint)
                
                '''
                CPorigin = selectedPlane.origin
                ui.messageBox(str(CPorigin)) 
                origin = adsk.core.Point3D.create(0,0,0)
                Pinsketch.sketchCurves.sketchLines.addByTwoPoints(CPorigin, origin)
                '''

            
            ListofSketches.append(sketch)
            ################################
            #ui.messageBox(str(len(sketch.profiles)))
            #Get the profile defined by the circle
            prof = sketch.profiles.item(0)
            real = float(CompHeights.pop())/10
            distance = adsk.core.ValueInput.createByReal(real)
            #Create an extrusion input
            extInput = extrudes.addSimple(prof, distance, adsk.fusion.FeatureOperations.NewBodyFeatureOperation)
            ################################
